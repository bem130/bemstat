...
特に自作言語を多く含む repository 群であるため、単純な拡張子別行数集計ではなく、言語ごとの分類処理を実装し、source、document、doc comment、test などの種類別統計を出せる構成にする。
...
- `docs/metrics/repo_metrics.json`
  - dashboard と外部処理向けの構造化データ。
- `docs/metrics/repo_metrics.csv`
  - 表計算や追加分析向けのフラットな CSV。
- `docs/metrics.html`
  - NEPLg2 の `metrics.html` と同系統の静的 dashboard。
- `docs/metrics/images/*.svg`
  - 主要グラフの静的 SVG。
- `docs/metrics/images/*.png`
  - 主要グラフの静的 PNG。
...
```text
section,owner,repository,name,language,files,lines,chars,bytes,blank,source,doc_comment,document,test,comment,other,test_cases
```
...
- `languages/generic.ts`
  - 汎用 source、document、other 用 classifier。
...
Markdown 系は `document`、`tests/` 配下は `test` を優先する。未知拡張子は `other` として集計し、`unknownExtensions` に出す。
...
- `owners-by-lines`
- `top-repositories-by-lines`
- `top-extensions-by-lines`
- `top-languages-by-lines`
- `content-kind-lines`

一方で、フィルタ、並べ替え、表示対象切り替えなどのインタラクティブなグラフは dashboard 側で `repo_metrics.json` を読み込んでブラウザ上で生成する。
...
- files、lines、bytes、source、test、document などの並べ替え
- 静的画像の閲覧セクション
...
`.github/workflows/pages.yml` を追加し、`main` push と `workflow_dispatch` で実行できるようにする。
...
- `docs/metrics.html` をローカルで開き、静的画像とインタラクティブチャートの両方が表示される。
